...
class SphereSpaceNavigation():
    ...
    def __init__(self,cspace_radius=5,q_start=(0,0),q_goal=(0,0),circular_obstacle_li=[],kappa=3):
        self.KAPPA = kappa
        ...
        ##########################################
        ############# make cspace ################
        self.X,self.Y,self.Z,self.mesh_shape,self.unit = MCS.get_cspace(xy_limit=6,step=0.01) 
        ...
        MCS.view_3d_surface(self.X,self.Y,self.Z,
                              ...
                              ,potential_field_title="Sphere Space Navigation Potential Field"#f"Sphere Space Navigation Potential Field\n KAPPA == {self.KAPPA}"
                            )
        """

    def get_coordinate_li(self):
        ##################################################################
        ###             MADE ON Friday March 5th
        # ................................................................
        UV = -1*self.path
        UVnan = UV/np.linalg.norm(UV,axis=0)
        # get where path has gradient
        pi, pj = ~np.isnan(UVnan[0]), ~np.isnan(UVnan[1])

        # indexed paths are not sorted by "next in path"
        xy = np.vstack([self.X[pi], self.Y[pj]]).T
        uv = np.vstack([UV[0,pi],UV[1,pj]]).T

        coordinate_li = []
        path_len = xy.shape[0]

        def get_and_pop_nearest_neighbor(cur, xy, uv):
          # get nearest neighbor
          nn_idx = np.argmin(np.linalg.norm(xy-cur,axis=1))
          cur_xy_uv = [xy[nn_idx],uv[nn_idx]]
          # "pop" array element
          xy, uv = np.delete(xy,nn_idx,axis=0),np.delete(uv,nn_idx,axis=0)
          return cur_xy_uv , xy , uv 

        # sort the path 
        cur_xy_uv, xy, uv  = get_and_pop_nearest_neighbor(self.q_start, xy, uv)
        while 0 != xy.shape[0] :
          cur_xy_uv, xy, uv  = get_and_pop_nearest_neighbor(cur_xy_uv[0], xy, uv)
          coordinate_li.append(cur_xy_uv)
    ...
    def calculate_navigation_path(self):
        ...
        path *= 0
        at_goal = lambda cur_gradient: 1.80715011e-02>np.linalg.norm(cur_gradient)

        seen_indices = []
        self.num_gradient_descent_epochs = 1
        gradient = self.gradient_vectors[:,i,j]
        while not at_goal(gradient):
            ###############################
            ...
            if (i,j) in seen_indices:
                ...
                SUCCESS_STR = "FAILURE"#f"FAILURE : Stuck at  {(self.X[i,j],self.Y[i,j])}"
                break   
...
if __name__ == "__main__" :
    ...
    SSN = SphereSpaceNavigation(q_start=start,q_goal=goal,circular_obstacle_li=obstacle_li,kappa=kappa)
    print(SSN.get_coordinate_li())
